...
- **`modules/cards/`** - Weather card renderers (`WeatherCards` facade)
  ...
  - `sun-times.js` - Sunrise & sunset card (side-by-side `fa-sunrise`/`fa-sunset` icons with times; skipped when both are missing, e.g. polar day/night). Titled with today's full locale date (e.g. "Wednesday, July 15, 2026 in Holland") next to a regional earth globe header icon (`fa-earth-americas`/`-africa`/`-asia`/`-europe`/`-oceania`, picked from the IANA timezone's continent prefix); its watermark omits the bottom-right timestamp (`drawWatermark(..., showTimestamp = false)`)
